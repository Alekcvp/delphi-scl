...
| Метод | Описание |
|-------|----------|
| `SetValue(...)` | Устанавливает значение (автоматически меняет тип) |
...
| `WrapWidth` | `Integer` | Ширина для переноса свёрнутых строк (минимум 40) |
...
- **Потокобезопасность** — не использует глобальное состояние
